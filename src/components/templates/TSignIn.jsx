...
const handleSubmit = async (e) => {
  ...
  try {
    const res = await axios.post('/api/auth/login/', { mobileNumber: phone });

    // بررسی وجود count در پاسخ
    const count = res?.data?.count;

    sessionStorage.setItem('UserNumber', phone);

    if (count === 1) {
      router.push('/Checkpass');
    } else {
      toast.success('درخواست با موفقیت ارسال شد!', { duration: 2000 });
      setSuccess(true);
      setTimeout(() => {
        router.push('/AutorazationForOtp');
      }, 2000);
    }
  } catch (err) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-white px-4">
      <div className="relative mb-10 select-none">
        <span className="text-5xl md:text-6xl font-bold text-black relative z-10" style={{ fontFamily: 'inherit' }}>
          ven<span className="inline-block relative">
            da
      ...
      <form className="w-full max-w-sm flex flex-col items-center gap-6" onSubmit={handleSubmit}>
        <label className="block w-full text-center text-gray-400 mb-2 text-base">شماره همراه خود را وارد کنید</label>
        <input
          type="tel"
          maxLength={11}
          pattern="09[0-9]{9}"
        ...
        {success && <div className="text-green-600 text-sm mt-2">درخواست با موفقیت ارسال شد!</div>}
      </form>

      <div className="mt-8 text-center text-sm text-gray-400">
        شرایط استفاده از <a href="#" className="text-pink-500 underline">قوانین و حریم خصوصی</a> وندا مد را می‌پذیرم
      </div>
    </div>
  );
}
